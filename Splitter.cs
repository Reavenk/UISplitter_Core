...
namespace PxPre.UIUtils
{
    ...
    public class Splitter : MonoBehaviour
    {
        ...
        { 
            ...
            Vector2 oldOffsetMin = this.rectTransform.offsetMin;
            Vector2 oldOffsetMax = this.rectTransform.offsetMax;
            this.rectTransform.pivot = new Vector2(0.0f, 1.0f);
            this.rectTransform.offsetMin = oldOffsetMin;
            this.rectTransform.offsetMax = oldOffsetMax;
        ...
        void Start()
        {
            ...
            if(_autoSplit)
                this.AutoSplit();
            else if(this.panes != null)
                this.SetPane(this.panes);
        }

        public void AutoSplit()
        { 
            // If we're scanning children of the system and we've clearing out previously
            // made splitter system, we need to get rid of sashes because they don't delete
            // fast enough to be fully deleted by the time we're iterating the new children.
            HashSet<GameObject> oldSashes = new HashSet<GameObject>();
            if(this.sashes != null)
            { 
                foreach(var kvp in this.sashes)
                    oldSashes.Add(kvp.Value.gameObject);
            }
            this.ClearSashes();
            ...
            { 
                if(!tChild.gameObject.activeInHierarchy || oldSashes.Contains(tChild.gameObject))
                    continue;

                RectTransform rt = tChild.GetComponent<RectTransform>();
                if(rt != null)
                    rts.Add(rt);
            }
            this.SetPane(rts);
